...
`kind:set`, `kind:str-building-commercial-skyscraper` and `kind:str-building-military-castle` are exempt. Of the rest, `tag:comp`, `tag:plural`, `tag:broken`, `tag:pickup` and `tag:piece` are exempt from the extents and only `tag:plural` from the budget, per measure in `lint/variables.json`.
