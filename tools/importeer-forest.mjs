...
for (const cel of gedeeld.cellen) {
  cel.bronnen = cel.bronnen.filter((b) => b.kit !== 'forest');
}
...
for (const [celSleutel, modellenSet] of perCel) {
  ...
  let bron = cel.bronnen.find((b) => b.kit === 'forest');
  if (!bron) {
    bron = { kit: 'forest', modellen: [] };
...
console.log('kits/palet.json bijgewerkt met forest-bronnen');
