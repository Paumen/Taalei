// Zet naar binnen kijkende vlakken naar een andere baan. Bij een afgebroken
// stronk of holle stam is de binnenkant blootliggend hout, geen schors.
// Splitst gedeelde hoekpunten zodat geen driehoek over twee cellen komt.
//
//   node tools/binnenwand.mjs <glb> --van 2,0 --naar 0,0 [--minhoogte 0.55] [--drempel 0.3] [--proef]
...
for (const mesh of glb.json.meshes ?? []) for (const prim of mesh.primitives ?? []){
  ...
  for(let t=0;t<idx.count;t+=3){
    ...
    const cx=d.reduce((s,i)=>s+P(i,0),0)/3-asx, cz=d.reduce((s,i)=>s+P(i,2),0)/3-asz;
    const rl=Math.hypot(cx,cz)||1;
    const radiaal=(n[0]/L)*(cx/rl)+(n[2]/L)*(cz/rl);
    const omlaag=-n[1]/L;
    // alleen echt naar binnen of recht omlaag; een schuin naar buiten vallend
    // vlak op de buitenkant mag niet mee
    if(radiaal > -drempel && omlaag < 0.85) continue;
    kies.add(t);
  }
