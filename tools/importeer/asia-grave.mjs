...
function bronModellen() {
  ...
  for (const map of mappen) {
    for (const bestand of readdirSync(map).filter((n) => n.toLowerCase().endsWith('.glb')).sort()) {
      ...
      for (const [naam, primitieven] of perMesh) modellen.push({ naam, primitieven });
    }
  }
...
function schrijf(pad, mesh, naam, bronmodel) {
  const stukken = [];
  const accessors = [];
  const bufferViews = [];
  let lengte = 0;
  ...
  const json = {
    asset: {
      generator: 'tools/importeer/asia-grave.mjs',
      version: '2.0',
      extras: { taaleiland: { versie: 1, schaal: SCHAAL, palet: 1, bron: BRON_NAAM, bronmodel } },
    },
    scene: 0,
    scenes: [{ nodes: [0] }],
...
for (const model of gekozen) {
  ...
  schrijf(join(doelMap, `${opgave.naam}.glb`), mesh, opgave.naam, model.naam);
  ...
  console.log(
    `${model.naam.padEnd(20)} → ${opgave.naam.padEnd(18)} ${mesh.driehoeken.length / 3} tris, ` +
...
zetTags(regels);
zetManifest(namen);
console.log(`\n${gekozen.length} models → kits/workfiles/${KIT}`);
