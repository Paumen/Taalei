...
render.mjs <file.glb|dir> [...] [flags]
  ...
  --sheet [--sheet-cols n] [--sheet-tile px] [--sheet-only]
                     ...
                     the pixels. Columns and cell size default to the fixed steps
                     below, so tiles stay comparable across sheets. Several models in
                     one run share one sheet, captioned per model; --sheet-each writes
...
// A sheet has no label strip, so the caption has to be in the tile itself.
if (opts.sheet && !opts.given.has('annotate')) opts.annotate = true;
...
// Sheet geometry by tile count: [cols, cell px]. Claude reads an image in 28 px
// patches, at most 2576 px on the long edge and 4784 patches per image (high-res
// tier, Claude 4.7 and later); anything bigger is downscaled on the way in. Cells are
// multiples of 28 so tile edges land on patch edges, and there are only five steps so
// the same tile keeps the same footprint across sheets: 1932 = 3x644, 1288 = 2x644.
// 3 tiles sit on the 4-column grid with one empty cell rather than on their own grid.
...
const VISION_EDGE = 2576, VISION_PATCHES = 4784, PATCH = 28;
function sheetGeometry(n, cols) {
  if (!cols) {
    if (n <= 1) return [1, 1932];
    if (n === 2) return [2, 1288];
    if (n <= 8) return [4, 644];
    if (n <= 16) return [4, 476];
    if (n <= 28) return [4, 364];
    cols = Math.ceil(Math.sqrt(n));
  }
  // caller-set columns, or more tiles than the table covers: largest patch-aligned
  // cell that keeps the sheet under both limits
  const rows = Math.max(1, Math.ceil(n / cols));
  const cell = PATCH * Math.floor(Math.min(VISION_EDGE / (PATCH * Math.max(cols, rows)),
    Math.sqrt(VISION_PATCHES / (cols * rows))));
  return [cols, Math.max(PATCH, cell)];
}
...
if (opts.ladder) {
  ...
  const [cols, cell] = sheetGeometry(opts.ladder, opts.sheetCols);
  if (untouched('sheetCols')) opts.sheetCols = cols;
  if (untouched('sheetTile')) opts.sheetTile = cell;
  if (untouched('width')) opts.width = opts.sheetTile;
  if (untouched('height')) opts.height = opts.sheetTile;
...
window.API = {
  ...
  async render(cfg) {
    ...
    if (cfg.compare) {
      ...
      if (cfg.keepTile) S.tiles.push({ bmp: await createImageBitmap(c2) });
    ...
    if (cfg.keepTile) S.tiles.push({ bmp: await createImageBitmap(src) });
  ...
  // Stack the row strips edge to edge and caption each model inside its own strip,
  // in the headroom above it. Captions are measured and placed first: each one drops
  // to the first line clear of its left neighbour, so a row of narrow parts does not
  ...
  async compareSheet(rowsMeta, cfg) {
    ...
    const rows = S.tiles.map((_, ri) => {
      ...
      return items.map(it => {
        ...
        const half = w / 2;
        const x = Math.min(Math.max((it.cx / cfg.rowW + 0.5) * tw, half + inset), tw - half - inset);
        let lv = 0;
        while (lv < rights.length && x - half < rights[lv] + gap) lv++;
        rights[lv] = x + half;
        return { it, x, w, level: lv };
      });
    });
    ...
    for (let ri = 0; ri < S.tiles.length; ri++) {
      ...
      for (const q of rows[ri]) {
        const lines = q.it.kit ? 2 : 1;
        const top = y + inset + q.level * lead * 2;
        g.fillStyle = 'rgba(255,255,255,0.86)';
        g.fillRect(q.x - q.w / 2 - inset * 0.5, top, q.w + inset, lead * lines + inset * 0.5);
        let ly = top + lead;
        if (q.it.kit) { g.font = kitFont; g.fillStyle = '#7a756d'; g.fillText(q.it.kit, q.x, ly); ly += lead; }
  ...
  tileCount() { return S.tiles.length; },

  // cols and cell are resolved by the driver (sheetGeometry); tiles butt edge to edge
  async sheet(cols, t, bg) {
    ...
    S.tiles.forEach((tile, i) => g.drawImage(tile.bmp, (i % cols) * t, Math.floor(i / cols) * t, t, t));
    return c.toDataURL('image/png').split(',')[1];
  }
};
window.__ready = true;
...
// smaller than its cell is upscaled here, so warn once when that happens.
let upscaleWarned = false;
async function writeSheet() {
  const n = await page.evaluate(() => window.API.tileCount());
  if (!n) return null;
  const [cols, auto] = sheetGeometry(n, opts.sheetCols);
  const cell = opts.sheetTile > 0 ? opts.sheetTile : auto;
  if (!upscaleWarned && Math.min(opts.width, opts.height) < cell) {
    upscaleWarned = true; warnings++;
    console.warn('  ! tiles render at ' + opts.width + 'x' + opts.height + ' but the sheet cell is '
      + cell + ' px; raise --width/--height or set --sheet-tile to avoid upscaling');
  }
  return page.evaluate(([c, t, bg]) => window.API.sheet(c, t, bg), [cols, cell, opts.bg]);
}
...
for (let i = 0; !opts.compare && i < models.length; i++) {
  ...
  try {
    ...
    if (runSheet) for (const j of jobs) j.caption = name + ' ' + j.caption;
