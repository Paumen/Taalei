...
- **S1.** A model tagged `special` is a deliberate exception, and no rule below
  applies to it — except the N4 ceiling. Only the PO assigns the tag; it is not a
  way to silence a finding.
...
- **M3.** Flowers, grass and plants never use dark green — that band is for trees
  and foliage. Theirs is light green.
...
- **M11.** Coins and jewellery metal are gold 6,0 or silver 3,2. Copper is
  terracotta 5,0. Sword and dagger grips are silver 3,2, so the grip reads apart
  from the blade.
...
- **M14.** Rope is taupe 14,3, never the light wood lane — it has to read apart from
  the planking it lashes. Where the model already holds taupe, N2's merge applies.
...
- **M25.** All cork is taupe 14,3 — stoppers and everything else cut from cork bark.
...
- **M27.** The bands on barrels, chests, buckets, trunks, kegs, crates and boxes are
  metal, and that metal is light grey 15,3 — whatever the model is made of.
...
- **M31.** A liquid takes its own colour, and it is dark red 8,0, dark green 1,1 or
  blue 4,2 — which one says which potion. Like M1 this is a rule of its own, within
  those three bands. Flask: M16. Stopper: M25.
- **M32.** Meat is terracotta 5,0, dark half 0.55-1.00, across the whole collection:
  joint, drumstick, ham, steak, roast bird. A cut or inner face stays in that range.
  Bone follows M6, the fat rim the off-white beside it.
- **M33.** What sits beside the meat is not meat, and M32 does not reach it: mash,
  carrots, potatoes, peas. Read it from the source model — on our palette mash and
  meat can both land on salmon 13,0, but the source pack says which is which.
  `food-stew` and `plate-food-b` hold no meat at all.
...
- **M35.** Leather is bark 2,0: belts, straps, boots, bracers, quivers, satchels and
  book covers are all one brown. That is why G1 lets a leather-look surface take the
  bark lane without a bark tag.
...
- **M38.** Timber is wood light 0,0 or wood middle 1,0; which one is G1. A model
  carrying timber shows at least one.
- **M39.** Bark is bark 2,0, never anything else. A trunk with a cut face carries
  timber too and shows both lanes (G1).
- **M40.** Foliage is dark green 1,1 or light green 3,1 — trees dark (M5), small
  flora light (M3, M4).
- **M41.** A gemstone carries the `gemstone` tag. Without it M36 can only excuse a
  band, never require one — and dark red, dark green and blue would fall back to
  excusing every model in coins-jewelry, a plain gold ring included.
...
- **C9.** Light green: nature only — flora and foliage, including grass and weed
  accents growing on objects and structures. Not a general accent band: green
  fletching, or a green stripe on something that grows nothing, is a finding.
...
- **C12.** Blue-grey 6,1: steel and cast iron (M10), worked stone (M20), wicks (M23),
  book covers (M37).
- **C13.** Clear glass: glass only. M17 makes it a material of its own, and nothing
  that is not glass is transparent.
...
- **C15.** Off-white 5,2 and taupe 14,3 carry no "only" clause — they are the two
  general-purpose bands, and a surface taking one is judged by its material's own
  M rule. The omission is deliberate: taupe alone is reached by sixteen of the
  nineteen materials.

### N. Counting — bands against materials
- **N1.** A model usually has at least one material.
- **N2.** A model usually uses at least as many bands as it has materials.
  Fewer is fine when two materials land on the same band — that is the merge
  working, not a fault. `village-kit/well-a` puts its rope on the taupe it
  already uses for stone.
...
- **N5.** Bring a model under the ceiling by merging bands, never by repainting.
  ...
  - The triangles moving onto it keep their gradient position, so the baked
    shading of G3 comes through intact.
  - Take the merge the eye does not catch. Merging away a two-tone the model is
    recognised by — the hood of `rogue-hooded`, the gold on a hilt beside its
    blade — costs more than it saves.
- **N6.** Every model named in `catalog/tags.json` exists in the catalogue.
  A tag pointing at a renamed or removed model is not harmless: the model it was
  meant for loses that tag silently, and an untagged model is one every M and C
  rule passes over without a word.
...
- **G4.** Which triangles are light or dark is stated in the source model. This is
  also in CLAUDE.md.
