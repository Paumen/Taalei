...
import { buildPalettes, paletteFindingsFor, idUnder } from './rules.mjs';
...
for (const m of models) {
  if (!m.kind || VARS.palette.exemptKinds.some((k) => idUnder(m.kind, k))) { skipped++; continue; }
  checked++;
  for (const f of paletteFindingsFor(m, PALETTES, materialIds, VARS)) {
...
const w = { id: width('id'), kind: width('kind'), material: width('material'), wants: width('wants') };
